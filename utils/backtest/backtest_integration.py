...
This module integrates vectorized strategies with the backtest engine,
providing comprehensive backtesting capabilities, parameter optimization,
and performance analysis for all trading strategies.
"""
...
from .indicator_factory import calculate_indicators

logger = logging.getLogger(__name__)

class StrategyBacktestSuite:
    """
    Comprehensive backtesting suite for all trading strategies
    ...
    def __init__(self, initial_capital: float = 10000.0, fees: float = 0.006, 
        ...
        logger.info(f"Strategy Backtest Suite initialized: ${initial_capital:,.2f} capital")
        logger.info(f"Results directory: {self.results_dir}")
    
    def run_comprehensive_backtest(self, data_with_indicators: pd.DataFrame, 
        ...
        try:
            ...
            for strategy_name, signals_df in all_signals.items():
                ...
                try:
                    ...
                    # Add market regime analysis if available
                    if 'market_regime' in signals_df.columns:
                        regime_analysis = self._analyze_strategy_by_regime(signals_df)
                        backtest_result.update(regime_analysis)
                    ...
                    results[strategy_name] = {'error': str(e), 'strategy_name': strategy_name}
            
            # Generate comparative analysis
            comparative_results = self._generate_comparative_analysis(results, product_id)
            
            # Store results
            self.backtest_results[product_id] = {
            ...
            self._save_results(product_id, self.backtest_results[product_id])
            
            logger.info(f"Comprehensive backtest completed for {product_id}")
            return self.backtest_results[product_id]
        ...
        try:
            ...
            for i, param_combo in enumerate(param_combinations):
                try:
                    ...
                    signals_df = self._generate_strategy_signals_with_params(
                        data_with_indicators, strategy_name, params, product_id
                    )
                    
                    if signals_df is None or signals_df.empty:
                        logger.warning(f"No signals generated for params {params}")
                        continue
                    
                    # Run backtest
                    backtest_result = self.backtest_engine.run_backtest(
                        data_with_indicators, signals_df, f"{product_id}-{strategy_name}-opt-{i}"
                    )
                    
                    # Add parameters to result
                    result = {**params, **backtest_result}
                    result['param_combination_id'] = i
        ...
        try:
            ...
            for i, (train_start, train_end, test_start, test_end) in enumerate(periods):
                try:
                    ...
                    test_signals = self._generate_strategy_signals_with_params(
                        test_data, strategy_name, best_params, f"{product_id}_wf_test_{i}"
                    )
                    
                    if test_signals is None or test_signals.empty:
                        logger.warning(f"No test signals for period {i+1}")
                        continue
                    
                    # Run backtest on test data
                    test_result = self.backtest_engine.run_backtest(
                        test_data, test_signals, f"{product_id}_wf_test_{i}"
                    )
    ...
    def _generate_strategy_signals_with_params(self, data: pd.DataFrame, strategy_name: str,
                                             params: Dict[str, Any], product_id: str) -> Optional[pd.DataFrame]:
        """Generate strategy signals with specific parameters"""
        try:
            # For now, we'll use the default strategy implementation
            # In a full implementation, this would modify strategy parameters
            
            if strategy_name == 'adaptive':
                return self.strategy_vectorizer.vectorize_adaptive_strategy(data, product_id)
            else:
                return self.strategy_vectorizer.vectorize_strategy(strategy_name, data, product_id)
            
        except Exception as e:
            logger.error(f"Error generating signals for {strategy_name} with params {params}: {e}")
            return None
    
    def _analyze_strategy_by_regime(self, signals_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            regime_analysis = {}
            regimes = signals_df['market_regime'].value_counts()
            
            for regime, count in regimes.items():
                regime_signals = signals_df[signals_df['market_regime'] == regime]
                buy_count = regime_signals['buy'].sum()
                sell_count = regime_signals['sell'].sum()
                avg_confidence = regime_signals['confidence'].mean()
                
                regime_analysis[f'{regime}_periods'] = count
                regime_analysis[f'{regime}_buy_signals'] = buy_count
                regime_analysis[f'{regime}_sell_signals'] = sell_count
                regime_analysis[f'{regime}_avg_confidence'] = avg_confidence
                regime_analysis[f'{regime}_signal_rate'] = (buy_count + sell_count) / count * 100
            
            return regime_analysis
            
        except Exception as e:
            logger.error(f"Error analyzing strategy by regime: {e}")
            return {}
    ...
    def _generate_comparative_analysis(self, results: Dict[str, Any], product_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            signal_analysis = {}
            for name, result in valid_results.items():
                total_signals = result.get('buy_signals', 0) + result.get('sell_signals', 0)
                signal_analysis[name] = {
                    'total_signals': total_signals,
                    'buy_signals': result.get('buy_signals', 0),
                    'sell_signals': result.get('sell_signals', 0),
                    'signal_rate': result.get('signal_count', 0) / len(results) * 100 if results else 0
                }
            ...
            return {
                ...
                'signal_frequency': signal_analysis,
                'summary': {
    ...
    def generate_performance_report(self, product_id: str) -> str:
        ...
        try:
            if product_id not in self.backtest_results:
                return f"No backtest results found for {product_id}"
            
            results = self.backtest_results[product_id]
            ...
            if 'risk_return_profile' in comparative:
                ...
                for profile in comparative['risk_return_profile']:
                    report.append(f"- **{profile['strategy']}**: {profile['return']:.2f}% return, "
                                f"{profile['risk']:.2f}% max drawdown, {profile['sharpe']:.3f} Sharpe")
                report.append("")
...
def run_strategy_backtest(data_with_indicators: pd.DataFrame, strategy_name: str,
    ...
    """
    suite = StrategyBacktestSuite(initial_capital=initial_capital)
    
    # Vectorize single strategy
    vectorizer = VectorizedStrategyAdapter()
    
    if strategy_name == 'adaptive':
        signals_df = vectorizer.vectorize_adaptive_strategy(data_with_indicators, product_id)
    else:
        signals_df = vectorizer.vectorize_strategy(strategy_name, data_with_indicators, product_id)
    
    if signals_df is None or signals_df.empty:
        return {'error': f'No signals generated for {strategy_name}'}
    
    return suite.backtest_engine.run_backtest(data_with_indicators, signals_df, f"{product_id}-{strategy_name}")
...
def compare_all_strategies(data_with_indicators: pd.DataFrame, product_id: str = "BTC-USD",
    ...
    """
    suite = StrategyBacktestSuite(initial_capital=initial_capital)
    return suite.run_comprehensive_backtest(data_with_indicators, product_id)